...
internal sealed class CategorizeChest : BaseFeature<CategorizeChest>
{
    ...
    private void OnItemTransferring(ItemTransferringEventArgs e)
    {
        ...
        if (e.IsAllowedByDefault && itemMatcher.IsEmpty)
        {
            e.AllowTransfer();
        }
        else if (!itemMatcher.IsEmpty && itemMatcher.MatchesFilter(e.Item))
        {
            e.AllowTransfer();
        }
        else if (e.IsAllowedByDefault && !itemMatcher.IsEmpty)
        {
            e.PreventTransfer();
        }
    }
